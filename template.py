...
def compare_models(prompt: str) -> dict:
    ...
    # Estimate cost: (words / 0.75) / 1000 * cost_per_1k_tokens
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    lines = []
    header = f"{'Prompt':<40} | {'GPT-4o':<20} | {'Mini':<20} | {'GPT-4o Lat':<8} | {'Mini Lat':<8}"
    lines.append(header)
    lines.append("-" * len(header))

    for r in results:
        prompt = r["prompt"][:37] + "..." if len(r["prompt"]) > 40 else r["prompt"]
        gpt4o = r["gpt4o_response"][:17] + "..." if len(r["gpt4o_response"]) > 20 else r["gpt4o_response"]
        mini = r["mini_response"][:17] + "..." if len(r["mini_response"]) > 20 else r["mini_response"]

        line = f"{prompt:<40} | {gpt4o:<20} | {mini:<20} | {r['gpt4o_latency']:<8.3f} | {r['mini_latency']:<8.3f}"
        lines.append(line)
